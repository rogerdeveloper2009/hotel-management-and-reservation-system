<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Add Room</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400">Create a room with pricing, capacity, status, and amenities.</p>
        </div>
    </x-slot>
        ...
        <form method="POST" action="{{ route('rooms.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div>
                    ...
                    <x-text-input id="floor" name="floor" type="number" class="mt-1 block w-full" :value="old('floor', 1)" required />
                    <x-input-error class="mt-2" :messages="$errors->get('floor')" />
                </div>
                <div>
                    <x-input-label for="capacity" value="Capacity" />
                    <x-text-input id="capacity" name="capacity" type="number" class="mt-1 block w-full" :value="old('capacity', 1)" required />
                ...
                <div>
                    ...
                    <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-white/10 dark:bg-white/5" required>
                        @foreach (['available','reserved','occupied','cleaning','maintenance'] as $s)
                            <option value="{{ $s }}" @selected(old('status', 'available') === $s)>{{ ucfirst($s) }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <x-text-input id="rate_per_night" name="rate_per_night" type="number" step="0.01" class="mt-1 block w-full" :value="old('rate_per_night')" required />
            ...
            <div>
                ...
                <select id="amenity_ids" name="amenity_ids[]" multiple class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-white/10 dark:bg-white/5">
                    @foreach ($amenities as $amenity)
                        <option value="{{ $amenity->id }}" @selected(collect(old('amenity_ids', []))->contains($amenity->id))>{{ $amenity->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                <x-input-label for="images" value="Room Images" />
            ...
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('rooms.index') }}" class="text-sm text-gray-600 hover:underline dark:text-gray-300">Cancel</a>
